...
- `npm run check:claims` (postbuild) — re-reads `dist/` and cross-checks numeric
  ...
  report a pass.
- `npm run check:urls` (postbuild) — canonical, `og:url` and sitemap agreement, and
  the social card: every page must carry the full Open Graph and X set, and each
  `og:image` has to resolve to a real image file in `dist/` at the dimensions it
  declares. A card naming a path that 404s is indistinguishable from a working one
  until somebody pastes the link, and by then the network has cached the miss.
  The site-default card is generated by `npm run og:image` from
  `src/assets/social/og-default.svg`; both the source and the PNG are committed, and
  no figure goes on it — it is cached by every network that scrapes it, so a number
  there could never be corrected.
